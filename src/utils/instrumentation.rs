...
/// Statistics collector for throughput samples
pub struct ThroughputStatsCollector {
    pub samples: Arc<std::sync::Mutex<Vec<Sample>>>,
    pub collector_handle: JoinHandle<()>,
    pub progress_handle: JoinHandle<()>,
}
...
impl ThroughputStatsCollector {
    pub fn new(
        ...
        start_time: Instant,
        duration: Duration,
    ) -> (Self, mpsc::UnboundedSender<Sample>) {
        ...
        let progress_handle = {
            let pb = progress_bar.clone();
            let samples_for_stats = samples.clone();
            tokio::spawn(async move {
                while start_time.elapsed() < duration {
                    let elapsed = start_time.elapsed().as_secs();
                    pb.set_position(elapsed);

                    if let Ok(samples) = samples_for_stats.try_lock()
                        && !samples.is_empty()
                    {
                        ...
                        let requests_per_sec = samples.len() as f64 / elapsed_secs;
    ...
    pub async fn finish(self, progress_bar: ProgressBar, message: String) -> Vec<Sample> {
        let _ = tokio::join!(self.collector_handle, self.progress_handle);
        progress_bar.finish_with_message(message);
