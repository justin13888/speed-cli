...
async fn measure_http_latency(client: &Client, url: &str, count: usize) -> Result<Option<LatencyResult>> {
    let mut measurements = Vec::new();
    
    println!("Measuring HTTP latency with {} requests...", count);
...
async fn run_download_test(client: &Client, config: &HttpTestConfig) -> Result<TestResult> {
    println!("Starting download test with {} parallel connections...", config.parallel_connections);
...
async fn run_upload_test(client: &Client, config: &HttpTestConfig) -> Result<TestResult> {
    println!("Starting upload test with {} parallel connections...", config.parallel_connections);
    ...
    let mut upload_data = vec![0u8; chunk_size];
    let mut rng = rng();
    ...
    for i in 0..config.parallel_connections {
        ...
        let task = tokio::spawn(async move {
            ...
            while connection_start.elapsed() < test_duration {
...
async fn determine_optimal_download_test_size(client: &Client, base_url: &str, parallel_connections: usize) -> Result<usize> {
    ...
    match download_chunk(client, &url).await {
        Ok(bytes) => {
            ...
            debug!("Estimated download speed: {:.2} Mbps, Optimal test size: {} bytes", mbps, optimal_size);
...
async fn export_http_results(result: &HttpTestResult, path: &str) -> Result<()> {
    if path.ends_with(".json") {
        ...
        let csv_content = format!(
            ...
            result.test_type,
            result.http_version,
            result.download_mbps.map_or("".to_string(), |v| v.to_string()),
...
fn print_http_results(result: &HttpTestResult) {
    ...
    println!("Parallel Connections: {}", result.parallel_connections.to_string().yellow());
    ...
    if let Some(download) = result.download_mbps {
        ...
        println!("Data Downloaded: {}", format_bytes(result.bytes_downloaded).yellow());
    }
    
    if let Some(upload) = result.upload_mbps {
        println!("Upload Speed: {}", format_bandwidth(upload).green().bold());
        println!("Data Uploaded: {}", format_bytes(result.bytes_uploaded).yellow());
    }
