...
pub async fn run_comprehensive_test(config: ComprehensiveTestConfig) -> Result<ComprehensiveTestResult> {
    println!("{}", "Starting comprehensive network diagnostic test...".green().bold());
    ...
    let overall_score = calculate_performance_score(&dns_result, &quality_result, &http_result, &topology_result);
    
    // Generate recommendations
    let recommendations = generate_recommendations(&dns_result, &quality_result, &http_result, &topology_result);
    ...
    println!("\n{}", format!("Comprehensive test completed in {:.2}s", total_time.as_secs_f64()).green().bold());
...
async fn test_dns_performance(server_url: &str) -> Result<DnsPerformanceResult> {
    ...
    println!("DNS resolution: {:.2}ms (IPv4: {}, IPv6: {})", 
             resolution_time, 
             if ipv4_support { "✓".green() } else { "✗".red() },
             if ipv6_support { "✓".green() } else { "✗".red() });
...
async fn test_connection_quality(server_url: &str) -> Result<ConnectionQualityResult> {
    ...
    let variance = latencies.iter().map(|&x| (x - avg_latency).powi(2)).sum::<f64>() / latencies.len() as f64;
...
fn generate_recommendations(
    ...
    if dns.resolution_time_ms > 100.0 {
        recommendations.push("Consider using faster DNS servers (e.g., 1.1.1.1 or 8.8.8.8)".to_string());
    }
    ...
    match quality.connection_stability {
        ConnectionStability::Poor => {
            recommendations.push("High jitter detected - check for network congestion or interference".to_string());
        },
        ConnectionStability::Fair => {
            recommendations.push("Moderate jitter detected - consider upgrading network equipment".to_string());
        },
        _ => {}
    }
    
    if quality.packet_loss_percent > 1.0 {
        recommendations.push("Packet loss detected - check network cables and equipment".to_string());
    }
    
    if let Some(download) = http.download_mbps
        && download < 25.0 {
            recommendations.push("Download speed below expectations - contact ISP or upgrade plan".to_string());
        }
    
    if let Some(upload) = http.upload_mbps
        && upload < 10.0 {
            recommendations.push("Upload speed may limit video conferencing and file sharing".to_string());
        }
...
fn print_comprehensive_results(result: &ComprehensiveTestResult) {
    println!("\n{}", "═".repeat(80).green());
    println!("{}", "COMPREHENSIVE NETWORK DIAGNOSTIC RESULTS".green().bold());
    ...
    println!("\n🎯 {} {:.1}/100", "Overall Score:".bold(), 
             result.overall_score.to_string().color(score_color).bold());
    
    // DNS Performance
    println!("\n{}", "🌐 DNS PERFORMANCE".cyan().bold());
    println!("   Resolution Time: {:.2}ms", result.dns_performance.resolution_time_ms);
    println!("   IPv4 Support: {}", if result.dns_performance.ipv4_support { "✓".green() } else { "✗".red() });
    println!("   IPv6 Support: {}", if result.dns_performance.ipv6_support { "✓".green() } else { "✗".red() });
    
    for dns_server in &result.dns_performance.dns_servers {
        let status = if dns_server.success { "✓".green() } else { "✗".red() };
        println!("   DNS Server {}: {:.2}ms {}", dns_server.server.cyan(), dns_server.response_time_ms, status);
    }
    
    // Connection Quality
    println!("\n{}", "📡 CONNECTION QUALITY".cyan().bold());
    println!("   Jitter: {:.2}ms", result.connection_quality.jitter_ms);
    println!("   Packet Loss: {:.2}%", result.connection_quality.packet_loss_percent);
    println!("   Stability: {:?}", result.connection_quality.connection_stability);
    println!("   Optimal MTU: {} bytes", result.connection_quality.mtu_discovery.optimal_mtu);
    
    // HTTP Performance
    println!("\n{}", "🚀 HTTP PERFORMANCE".cyan().bold());
    if let Some(download) = result.http_performance.download_mbps {
        println!("   Download Speed: {}", format_bandwidth(download).green().bold());
    }
    ...
    println!("   Parallel Connections: {}", result.http_performance.parallel_connections);
    
    // Network Topology
    println!("\n{}", "🗺️  NETWORK TOPOLOGY".cyan().bold());
    println!("   Hop Count: {}", result.network_topology.hop_count);
    println!("   Route Stability: {}", if result.network_topology.route_changes { "Unstable".red() } else { "Stable".green() });
