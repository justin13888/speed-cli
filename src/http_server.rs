...
pub async fn run_http_server(config: HttpServerConfig) -> Result<()> {
    ...
    println!("{}", format!("HTTP speed test server listening on {addr}").green().bold());
        ...
        Ok(collected) => {
            ...
            if size > config.max_upload_size {
                return Response::builder()
                    ...
                    .body(Full::new(Bytes::from(format!("Upload too large: {size} bytes"))))
                    .unwrap();
            }
            
            println!("Received {} of upload data", crate::network::format_bytes(size as u64).yellow());
...
async fn handle_latency(
    ...
    Response::builder()
        .status(StatusCode::OK)
        .header("Content-Type", "application/json")
        .header("Content-Length", response_data.to_string().len().to_string())
        .body(Full::new(Bytes::from(response_data.to_string())))
        .unwrap()
}
